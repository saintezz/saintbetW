...
// Миграции для существующих баз данных SQLite
...
// Активации промокодов пользователями
db.exec(`
...
app.post("/api/admin/grant-bones", (req, res) => {
    try {
        ...
        if (!target_username || !amount) {
            return res.status(400).json({
                success: false,
                error: "target_username и amount обязательны"
            });
        }
        ...
        `).get(String(target_username).replace(/^@/, ""));

        if (!user) {
            return res.status(404).json({
                success: false,
                error: `Пользователь @${target_username} не найден`
            });
        }

        db.prepare(`
            UPDATE users SET balance = balance + ? WHERE telegram_id = ?
        `).run(amt, user.telegram_id);

        const updated = db.prepare(`SELECT * FROM users WHERE telegram_id = ?`).get(user.telegram_id);

        res.json({
            success: true,
            message: `Выдано ${amt} костей пользователю @${target_username}`,
            user: updated
        });
...
app.post(["/api/promo/redeem", "/api/promo-code/activate"], (req, res) => {
    try {
        ...
        if (!telegramId) {
            telegramId = req.headers["x-user-id"] || "user_" + (req.ip || "local").replace(/[^a-zA-Z0-9]/g, "");
        }

        if (req.body.username && !username) {
            username = String(req.body.username).trim().toLowerCase();
        }
        if (req.body.first_name && !firstName) {
        ...
        if (promo.expires_at > 0 && Date.now() > promo.expires_at) {
        ...
        // 5. Проверка, активировал ли уже этот пользователь
        const alreadyUsed = db.prepare(`
            SELECT id FROM promo_activations WHERE code = ? AND telegram_id = ?
        `).get(codeUpper, telegramId);

        if (alreadyUsed) {
            return res.status(409).json({
                success: false,
                error: "Ты уже активировал этот промокод"
            });
        }
        ...
        const tx = db.transaction(() => {
            ...
            db.prepare(`
                INSERT INTO promo_activations (code, telegram_id)
                VALUES (?, ?)
...
app.post(["/api/admin/promos", "/api/admin/promo-codes"], (req, res) => {
    try {
        ...
        if (!code || !String(code).trim()) {
            return res.status(400).json({
                success: false,
                error: "Укажите название промокода"
            });
        }
        ...
        if (isNaN(acts) || acts < 1) {
            return res.status(400).json({
                success: false,
                error: "Количество активаций должно быть больше 0"
            });
        }
        ...
        if (isNaN(rew) || rew < 1) {
            return res.status(400).json({
                success: false,
                error: "Награда должна быть больше 0"
            });
        }

        let expiresAt = 0;
        if (!isNaN(minutes) && minutes > 0) {
            expiresAt = Date.now() + Math.floor(minutes * 60 * 1000);
        }
        ...
        res.json({ 
            ...
            promo: newPromo,
            promoCode: newPromo,
            message: `Промокод ${codeUpper} успешно создан!`
        });
...
app.delete(["/api/admin/promos/:id", "/api/admin/promo-codes/:id"], (req, res) => {
    try {
        ...
        if (promo && promo.code) {
            try {
                db.prepare(`DELETE FROM promo_activations WHERE code = ?`).run(promo.code);
            } catch(e) {}
        }
...
// =========================================================
// 404 — всегда JSON (не HTML)
// =========================================================

app.use((req, res) => {
    res.status(404).json({
        success: false,
        error: `Endpoint не найден: ${req.method} ${req.path}`
    });
});
